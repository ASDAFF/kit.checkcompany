...
class Eventhandler {
    ...
    public function OnBuildGlobalMenuHandler(&$arGlobalMenu, &$arModuleMenu){
        \Sotbit\Checkcompany\Helper\Menu::getAdminMenu($arGlobalMenu, $arModuleMenu);
    }

    public function onEpilogHandler()
    {
        $setLimit = \COption::GetOptionString("sotbit.checkcompany", 'EXECUTE_ON_SEPARATE_PAGES', "N");

        if ($setLimit == "Y") {
            global $APPLICATION;
            $url = $APPLICATION->GetCurPage(false);
            $listUrl = array_diff(unserialize(\COption::GetOptionString("sotbit.checkcompany", 'URL_TO_EXECUTE')), array(''));
            $isTemplate = self::checkTemplate($listUrl, $url);
        }

        if ($setLimit == "N" || $isTemplate) {
            \CModule::includeModule('sale');
            $persons = unserialize(\COption::GetOptionString("sotbit.checkcompany", 'PERSON_TYPES'));
            if (!is_array($persons)) {
                $persons = array();
            }
            $data = array();
            ...
            if (count($persons) > 0 && !strstr($APPLICATION->GetCurPage(), 'bitrix')) {
                foreach ($persons as $item) {
                    $inn = \COption::GetOptionString("sotbit.checkcompany", 'INN_' . $item);
                    $kpp = \COption::GetOptionString("sotbit.checkcompany", 'KPP_' . $item);
                    $ogrn = \COption::GetOptionString("sotbit.checkcompany", 'OGRN_' . $item);
                    $company = \COption::GetOptionString("sotbit.checkcompany", 'COMPANY_' . $item);
                    $address = \COption::GetOptionString("sotbit.checkcompany", 'ADDRESS_' . $item);
                    $f = \COption::GetOptionString("sotbit.checkcompany", 'F_' . $item);
                    $i = \COption::GetOptionString("sotbit.checkcompany", 'I_' . $item);
                    $o = \COption::GetOptionString("sotbit.checkcompany", 'O_' . $item);
                    $input = \COption::GetOptionString("sotbit.checkcompany", 'INPUT_' . $item);
                    $items = array();
                    $codes = array();
                    if ($inn != '-') {
                        $codes[] = $inn;
