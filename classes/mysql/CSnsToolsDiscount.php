...
{  
    ...
    function Add($arFields)
    {
        ...
        $arInsert = $DB->PrepareInsert("b_snstools1c_discont", $arFields);
        $strSql =
            "INSERT INTO b_snstools1c_discont(".$arInsert[0].") ".
            "VALUES(".$arInsert[1].")";
    ...
    function Update($ID ,$arFields)
    {
        ...
        $ID = IntVal($ID);  
        $strUpdate = $DB->PrepareUpdate("b_snstools1c_discont", $arFields);  
        $strSql = "UPDATE b_snstools1c_discont SET ".$strUpdate." WHERE ID=".$ID;
    ...
    function GetIdByDiscontId($ORDER_ID)
    {
        ...
        $strSql = "SELECT P.ID FROM b_snstools1c_discont P WHERE P.DISCONT_ID='".$DB->ForSql($ORDER_ID)."'";
    ...
    function GetById($ID)
    {
        ...
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_snstools1c_discont P WHERE P.ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function GetByDiscontId($ID)
    {
        ...
        $ORDER_ID = IntVal($ID);
        $strSql = "SELECT P.* FROM b_snstools1c_discont P WHERE P.DISCONT_ID='".$DB->ForSql($ID)."'";
        $dbRes = $DB->Query($strSql, true);
        $arRes = $dbRes->Fetch();
        return $arRes;        
    }      
    ...
    function Delete($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_snstools1c_discont WHERE ID='".$DB->ForSql($ID)."'";
    ...
    function DeleteByDiscontId($ID)
    {
        global $DB;
        $ID = IntVal($ID);
        $strSql = "DELETE FROM b_snstools1c_discont WHERE DISCONT_ID='".$DB->ForSql($ID)."'";
        $DB->Query($strSql, true);
        return true; 
    }         
