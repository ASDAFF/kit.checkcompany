...
class CModuleOptions
{
    ...
    public function CModuleOptions($module_id, $arTabs, $arGroups, $arOptions, $need_access_tab = false)
    {
        ...
        if($need_access_tab) {
            $this->arTabs[] = array(
                'DIV' => 'edit_access_tab',
                'TAB' => GetMessage($module_id."_access_Tab"),
                'ICON' => '',
                'TITLE' => GetMessage($module_id."_access_title")
            );
        }
    ...
    private function SaveOptions()
    {
        ...
        foreach($this->arOptions as $opt => $arOptParams)
        {
            ...
            else if ($opt == 'URL_TO_EXECUTE'){
                if($_REQUEST["URL_TO_EXECUTE"]){
                    COption::SetOptionString( $this->module_id, "URL_TO_EXECUTE", serialize( $_REQUEST["URL_TO_EXECUTE"] ), $_GET['site']);
                }
            }
        }
    }
                ...
                switch($arOptParams['TYPE'])
                {
                    ...
                    case 'COLORPICKER':
                        ...
                        $APPLICATION->IncludeComponent('bitrix:main.colorpicker', '', Array(
                            'SHOW_BUTTON' => 'Y',
                            'ID' => $opt,
                            'NAME' => GetMessage( $this->module_id."_choice_color"),
